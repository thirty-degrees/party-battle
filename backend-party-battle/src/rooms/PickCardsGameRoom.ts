...
export class PickCardsGameRoom extends BaseGameRoom<PickCardsGameSchema> {
  ...
  private setCurrentPlayer() {
    if (this.state.inGamePlayers.length > 0) {
      this.state.currentPlayer =
        this.state.inGamePlayers[this.currentPlayerIndex].name;
    }
  }
  ...
  private startPlayerTurn() {
    ...
    this.state.timeWhenTimerIsOver =
      this.clock.currentTime + this.playerTurnTimeout;
  ...
  private advanceToNextPlayer(_options: {
    lobbyRoomId: string;
    playerNames: string[];
  }) {
    this.clearPlayerTurnTimer();
    this.currentPlayerIndex =
      (this.currentPlayerIndex + 1) % this.state.inGamePlayers.length;
    this.setCurrentPlayer();
    this.startPlayerTurn();
  }
  ...
  private getPlayerNameBySessionId(sessionId: string): string | null {
    for (const [
      playerName,
      playerSessionId,
    ] of this.playerConnections.entries()) {
  ...
  private handleHotCardPressed(playerName: string) {
    ...
    const playerIndex = this.state.inGamePlayers.findIndex(
  ...
  override getScores(): Score[] {
    ...
    if (this.state.inGamePlayers.length > 0) {
      const remainingPlayers = this.state.inGamePlayers.map(
        (player) => player.name
      );
